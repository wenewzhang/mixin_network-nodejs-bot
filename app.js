...
mixin.onMessage = (data) => {
  mixin.decode(data).then(function(msgobj) {
    ...
    if (msgobj.action && msgobj.action != 'ACKNOWLEDGE_MESSAGE_RECEIPT' &&
      msgobj.action != 'LIST_PENDING_MESSAGES') {

      mixin.sendMsg("ACKNOWLEDGE_MESSAGE_RECEIPT", {
        message_id: msgobj.data.message_id
      }).then(function(receipt_id) {
        console.log("send ACKNOWLEDGE_MESSAGE_RECEIPT id:" +
          receipt_id);
      });
    } else {
      console.log(msgobj);
    ...
    if (msgobj.data.data == "hi") {
      mixin.sendText("Payment:", msgobj).then(function(receipt_id) {
        let payLink = "https://mixin.one/pay?recipient=" + config.mixin
          ...
          mixin.newuuid() + "&memo=";
        let btn = '[{"label":"pay 10 CNB","action":"' + payLink +
          '","color":"#ff0033"}]'
      ...
      fs.readFile("snowboarding-solid-svg.png", function(err, data) {
        ...
        mixin.sendImage(base64Image, msgobj).then(function(receipt_id) {
          let payLink = "https://mixin.one/pay?recipient=" +
            config.mixin.client_id + "&asset=" + CNB +
            "&amount=10" + '&trace=' + mixin.newuuid() + "&memo=";
          let btn = '[{"label":"pay 10 CNB","action":"' + payLink +
            '","color":"#ff0033"}]'
  ...
  return new Promise((resolve, reject) => {
    if (msgobj.action == 'CREATE_MESSAGE') {
      if (msgobj.data.category == 'PLAIN_TEXT') {
        let msg = Buffer.from(msgobj.data.data, 'base64').toString(
          'utf-8');
